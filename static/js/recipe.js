function Recipe(){
    ...
    this.render_summary = function(){
        var $recipe = $("<article class='recipe card'></article>");
        ...
        $recipe.addClass(this.class_name);
        $recipe.attr("id", this.store_id);

        $holder.append($section1, $section2, $section3);
        $recipe.append($holder);
        this.$holder.append($recipe);
    }
}


Recipe.fetch_data = function(){
    var url = "http://"+server+"/popup/rest/recipes.php";
    $.getJSON(url)
    .done(function(data){
